...
describe('validateRoomCode', () => {
  ...
  it.each([
    ...
    [200, { phase: 'ended' }, { ok: false, reason: 'ended' }],
...
describe('roomErrorMessage', () => {
  it.each([
    ...
    expect(roomErrorMessage(reason as 'ended' | 'not_found')).toContain(expected);
  });
});
...
describe('matchNewRoomCode', () => {
  ...
  it.each([
    [
      'refresh succeeds but lobbyCode missing',
    ...
    [
      ...
      [
        new Response('', { status: 401 }),
        new Response(JSON.stringify({ refreshed: false }), { status: 401 }),
      ],
      false,
    ],
    [
