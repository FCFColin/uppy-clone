...
/** Preserve current balloon/ghost positions when the game ends (no teleport to spawn). */
export function freezeInterpolation(): void {
  const bx: number = state.balloon.x;
  const by: number = state.balloon.y;
  ...
  currSnapshotTime = now;
  console.log(`[interp] interpolation FROZEN at balloon=(${bx.toFixed(4)},${by.toFixed(4)})`);
}
