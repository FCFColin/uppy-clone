...
describe('ensureAuth', () => {
  ...
  it('returns false on rate limit', async () => {
    sessionMocks.establishGameSession.mockResolvedValue({ ok: false, reason: 'rate_limit' } as import('../shared/session.js').SessionResult);
    const err = vi.spyOn(console, 'error').mockImplementation(() => {});
    await expect(ensureAuth()).resolves.toBe(false);
    err.mockRestore();
  });
  ...
  it('returns false on network error', async () => {
    sessionMocks.establishGameSession.mockResolvedValue({ ok: false, reason: 'network' } as import('../shared/session.js').SessionResult);
    const err = vi.spyOn(console, 'error').mockImplementation(() => {});
    await expect(ensureAuth()).resolves.toBe(false);
    err.mockRestore();
  });
