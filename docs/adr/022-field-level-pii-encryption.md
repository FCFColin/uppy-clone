...
**负面**
- `RotateKey()` 已实现（`aes.go:212-220`）：激活新密钥用于后续加密，配合 `ReEncryptWithKey` 遍历已有密文完成全量轮换
- `EncryptEmailForStorage` 在 `encKey == nil` 时回退明文（`aes_email.go:38-43`），仅 dev 安全
- 加密字段无法直接 SQL `LIKE` 查询
- `AUDIT_SECRET` 当前可回退到 `JWT_PRIVATE_KEY`（`config.Env.GetAuditSecret()`），审计完整性与签名密钥耦合
