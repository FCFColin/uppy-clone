...
> **合并说明（2026-07-22）**：本 ADR 已合并原 ADR-027《Room 出站管道（锁外广播与异步持久化）》
> 的全部内容。ADR-027 与本 ADR 决策 2 高度重叠（同一出站管道与异步持久化方案），其唯一性内容
> （数据流图、风险/缓解详表）已并入本 ADR 决策 2。ADR-027 文件已删除，本 ADR 为 Room 出站管道的唯一权威记录。

## 上下文

Hub 内存管理所有房间状态（`map[string]*Room`），存在单实例限制、故障风险（实例故障房间丢失）、tick 路径阻塞（持锁同步执行 WS 广播 + Redis Pub/Sub + PG 持久化）。Room goroutine 在 15Hz tick 路径上持 `Room.mu` 期间曾同步执行：

- WebSocket 广播（含慢客户端 `time.After` 等待）
- Redis Pub/Sub 发布（100ms 超时）
- PostgreSQL `SaveLobbyState`（每 2s）

高 PG/Redis 延迟或多玩家广播时，tick 与 `HandleMessage` 互相阻塞，导致 mass disconnect 与输入卡顿。多区域路由层已实现（ADR-014：`/resolve` + `room_directory` + owner 反向代理 + `ClaimRoomOwnership`），支持多实例水平扩展。单实例关闭 `EnableMultiRegion` 即可。
...
**正面**：`room_lock_hold_seconds` P95 < 10ms（正常依赖延迟下）；PG/Redis 抖动不再直接拖慢 15Hz tick；可观测 `room_outbound_queue_depth`、`room_persist_lag_seconds` metrics。
...
## 关联: ADR-006（Redis 策略）、ADR-007（异步处理与 Outbox）、ADR-014（多区域部署）。原 ADR-027（Room 出站管道）已合并至此（2026-07-22）；ADR-009 Transactional Outbox 已被 ADR-033 决策 2 豁免，EndGame 结果改为直接写 PG，不再经 outbox。
