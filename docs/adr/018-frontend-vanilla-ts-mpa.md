...
- 与后端共享物理常量（`shared/constants.ts` ↔ `protocol/constants.go`）
...
- 状态管理：可变单例 `state` 对象（`frontend/src/game/state.ts:71-92`），无 Redux/Zustang
...
**负面**
- WebSocket 客户端已拆分为 `ws_connect.ts`、`ws_handlers_*.ts` 等模块（原 `websocket.ts` 为薄 re-export 层），但连接/协议/重连/UI 仍跨模块协作，缺乏框架的组件边界
- 无声明式 UI，DOM 操作分散在 `ui.ts` 和各 entry 文件中
- 前端测试覆盖极低（仅 2 个 vitest 文件 vs ~22 源文件）
