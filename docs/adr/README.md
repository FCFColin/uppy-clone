...
| 005 | Hub 房间状态管理与出站管道（← 合并 027） | 已接受 | 2026-06 |
| 006 | Redis 策略（缓存与域拆分）（← 合并 029） | 已接受 | 2026-07 |
| 007 | 异步处理策略（Outbox + 消息队列）（← 合并 009） | 已接受 | 2026-06 |
...
为消除重叠冗余，以下 ADR 已合并入更低编号的权威 ADR，源文件已删除：

| 已删除 | 合并入 | 主题 | 说明 |
|--------|--------|------|------|
| 009 | 007 | 事务性 Outbox 模式 | 与 007 异步处理策略重叠；009 的 At-Least-Once 语义、消费者幂等契约、重试/退避、监控指标已并入 007 决策 1 |
| 027 | 005 | Room 出站管道 | 与 005 决策 2 高度重叠；027 的数据流图、风险/缓解详表已并入 005 决策 2 |
| 029 | 006 | Redis 域拆分策略 | 是 006 决策 2 的实现层细化；029 的三阶段策略、物理隔离实现、环境变量、理由权衡已并入 006 决策 2 |

> 注：ADR-032 与 ADR-033（瘦身豁免）被多个 ADR 引用为真实决策，但其文件不在 `docs/adr/` 目录中，
> 属预先存在的不一致（索引表行数比实际文件多 2）。本次整合未改动这两条记录，留待后续补齐文件或清理索引。
